  ...
  create(mailboxId, source, input) {
    ...
    return this.transaction(() => {
      ...
      this.db.prepare(`INSERT INTO mail_send_drafts
        (draft_id,mailbox_id,request_id,source,message_id,to_json,cc_json,subject,body_text,payload_digest,status,created_at,digest_version,links_json)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
        id, mailboxId, input.request_id, source, payload.message_id,
