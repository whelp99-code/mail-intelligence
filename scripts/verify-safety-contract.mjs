...
assert.match(serverSource, /const OUTLOOK_OAUTH_REDIRECT_URI = 'http:\/\/localhost:3010\/auth\/callback';/);
assert.match(serverSource, /function redirectUri\(\) \{\s+return OUTLOOK_OAUTH_REDIRECT_URI;\s+\}/s);
assert.doesNotMatch(serverSource, /function redirectUri\(req\)/);
assert.match(serverSource, /redirectUri:\s*oauthRedirectUri/);
assert.match(serverSource, /authorize\.searchParams\.set\('redirect_uri', oauthRedirectUri\)/);
assert.match(serverSource, /redirect_uri:\s*pending\.redirectUri/);
