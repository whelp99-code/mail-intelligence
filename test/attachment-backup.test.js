...
test('verify:backup:isolated passes against a synthetic schema-7 fixture, not the operational DB', async (t) => {
  ...
  assert.match(result.stdout, /"isolatedBackupRestore": "PASS"/);
  assert.match(result.stdout, /"schemaVersion": 7/);
  assert.match(result.stdout, /"liveDatabaseReplaced": false/);
});
